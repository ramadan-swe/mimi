...
export default function FilterSidebar({ filters, onFilterChange }) {
  ...
  return (<Card className="p-6 space-y-6">
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {TRANSMISSION_TYPES.map((type) => (<div key={type.value} className="flex items-center space-x-2">
          <Checkbox id={`transmission-${type.value}`} checked={selectedTransmission.includes(type.value)} onCheckedChange={() => handleCheckboxChange(type.value, selectedTransmission, setSelectedTransmission)} />
          <label htmlFor={`transmission-${type.value}`} className="text-sm cursor-pointer flex-1">
            {type.label}
          </label>
        </div>))}
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {FUEL_TYPES.map((type) => (<div key={type.value} className="flex items-center space-x-2">
          <Checkbox id={`fuel-${type.value}`} checked={selectedFuelTypes.includes(type.value)} onCheckedChange={() => handleCheckboxChange(type.value, selectedFuelTypes, setSelectedFuelTypes)} />
          <label htmlFor={`fuel-${type.value}`} className="text-sm cursor-pointer flex-1">
            {type.label}
          </label>
        </div>))}
      </div>
    ...
    <div className="space-y-3">
      ...
      <div className="space-y-2">
        {CATEGORIES.map((category) => (<div key={category.value} className="flex items-center space-x-2">
          <Checkbox id={`category-${category.value}`} checked={selectedCategories.includes(category.value)} onCheckedChange={() => handleCheckboxChange(category.value, selectedCategories, setSelectedCategories)} />
          <label htmlFor={`category-${category.value}`} className="text-sm cursor-pointer flex-1">
            {category.label}
          </label>
        </div>))}
      </div>
